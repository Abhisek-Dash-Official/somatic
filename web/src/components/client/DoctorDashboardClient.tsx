...
import { Activity, Users, CheckCircle, AlertTriangle, Loader2, Power } from "lucide-react";
...
export default function DoctorDashboardClient() {
    ...
    if (user?.role !== "doctor") {
        return <div className="text-center py-20 text-red-400">Access Denied. Doctor privileges required.</div>;
    }
        ...
        <div className="animate-in fade-in zoom-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Here is your clinical overview for today.</p>
                </div>
            ...
            <div className="bg-[#131C31] rounded-2xl border border-slate-800/80 shadow-xl overflow-hidden">
                <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-[#0d1425]">
                    ...
                    <span className="bg-blue-500/20 text-blue-400 text-xs px-3 py-1 rounded-full font-semibold border border-blue-500/30">
                        {data?.activeCases.length} Pending
                    </span>
                </div>
                                ...
                                <div key={caseItem._id} className="p-6 hover:bg-[#1a2642] transition-colors flex flex-col md:flex-row justify-between items-start md:items-center gap-4 group">
                                    <div>
                                        <div className="flex flex-wrap items-center gap-3 mb-2">
                                            ...
                                            {isUnclaimed ? (
                                                <span className="text-yellow-400 text-xs font-semibold bg-yellow-500/10 border border-yellow-500/20 px-2 py-1 rounded">
                                                    New (Department)
                                                </span>
                                            ) : (
                                                <span className="text-green-400 text-xs font-semibold bg-green-500/10 border border-green-500/20 px-2 py-1 rounded">
                                                    My Ongoing Case
                                                </span>
                                            )}
